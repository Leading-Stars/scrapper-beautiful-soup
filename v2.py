...
def get_first_text(soup, selectors, filter_invalid=True):
    ...
    invalid_keywords = {"photos", "write", "add", "videos", "menu", "share", "edit", "more"}
...
async def scrape_place_details(html: str) -> dict:
    ...
    if rating_block:
        full_match = re.search(r'(\d+(?:\.\d+)?)\sstars\s(\d+)\sReviews', rating_block)
        if full_match:
            rating = full_match.group(1)
            review_count = full_match.group(2)
        else:
            short_match = re.search(r'(\d+(?:\.\d+)?)$$(\d+)$$', rating_block)
            if short_match:
